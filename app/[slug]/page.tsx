...
function ResourceLinkGroups({items}:{items:string[]}) {
  const internal = items.map(item=>item.match(/^(.+) — (\/\S+)$/)).filter((match):match is RegExpMatchArray=>Boolean(match));

  return <div className="resource-link-groups">
    <ul className="resource-inline-links">
      {internal.map(match=><li key={match[2]}><Link className="resource-inline-link" href={match[2]}><span aria-hidden="true">◆</span><span>{match[1]}</span><span aria-hidden="true">→</span></Link></li>)}
    </ul>
    <div className="resource-featured-links">
      <FeaturedExternalLink href="https://www.moon-doll.com/" variant="moon" icon="☾" title="Открыть Moon-Doll" subtitle="внешний международный магазин" rel="noopener noreferrer sponsored"/>
      <FeaturedExternalLink href="https://t.me/rudollforum" variant="telegram" icon="✈" title="Telegram Rudollforum" subtitle="канал и вход в сообщество"/>
    </div>
  </div>;
}
